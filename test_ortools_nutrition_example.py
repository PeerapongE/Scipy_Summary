# -*- coding: utf-8 -*-
"""
Created on Mon Apr  9 15:53:55 2018
...
# LP problem
solver = pywraplp.Solver('SolveLP_Problem',pywraplp.Solver.GLOP_LINEAR_PROGRAMMING) # Name_Solver, Algorithm
...
for i in range(0, len(data)):
    ...
    food[i] = solver.NumVar(0.0, solver.infinity(), data[i][0]) # Set  Vairable Min, Max, Name_Variable: LP
    #food[i] = solver.IntVar(0.0, solver.infinity(), data[i][0]) # Set  Vairable Min, Max, Name_Variable: MILP
    
    #objective.SetCoefficient(food[i], 1) # This is not take price per unit into account
    objective.SetCoefficient(food[i], data[i][2]) # This is to take community price per unit
...
for i in range(0, len(nutrients)): # For each constraints nutrients
    constraints[i] = solver.Constraint(nutrients[i][1], solver.infinity()) # Nutritient constrant [var, inf] or >= var
    for j in range(0, len(data)): # For each food data, set coefficient for each nutrients
...
if status == solver.OPTIMAL:
    ...
    for i in range(0, len(data)):
        price += food[i].solution_value() * data[i][2] # Amount * unit_price
        ...
        #print "%s = %f" % (data[i][0], food[i].solution_value())
            print(' Ingredrient : ' + str(data[i][0]) + '\t Unit = '+ str(food[i].solution_value()))
    print ('Optimal price: is ' +  str(price))
